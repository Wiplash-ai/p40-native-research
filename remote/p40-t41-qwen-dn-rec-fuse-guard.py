#!/usr/bin/env python3
"""Forced-command identity for exact Qwen DeltaNet recurrence fusion."""
...
SPEC = importlib.util.spec_from_loader("p40_t41_base", SourceFileLoader("p40_t41_base", str(SOURCE)))
...
OUTPUT_TOKENS = 64
EXPECTED_ORIGINAL_COMMAND = "p40-t41-qwen-dn-rec-fuse"
RESULTS_ORIGINAL_COMMAND = "p40-t41-qwen-dn-rec-fuse-results"
ENGINE = Path("/home/jordanculver/p40-native-research/colibri-t41-dn-rec-fuse/c/qwen36")
ENGINE_SHA256 = "13b7c8c13dd2e5146f2805437c2a072a164a0f1b4180da5f331e7bcfb203ce6c"
RESULT_DIRECTORY = Path("/home/jordanculver/p40-native-research/results/t41-qwen-dn-rec-fuse")
PROFILE_ID = "t41-exact-qwen-dn-rec-fuse-64"
SCHEMA_VERSION = "p40-t41-qwen-dn-rec-fuse-v1"
EXPECTED_STDOUT_SHA256 = "5909fad89de2faac1b77af72bf8b25f56b8f33853df59d35cb04120e9ce1f35f"
ACTIVE_MARKER = b"[dn-rec-fuse] active: scale+kv and update+out state passes fused"
...
def model_argv() -> list[str]:
    ...
    for argument in _base_model_argv():
        ...
        if argument == "COLI_CUDA_DN_PAIR=1":
            result.append("COLI_DN_REC_FUSE=1")
    return result
...
def run(request: dict) -> dict:
    ...
    if not request["dry_run"] and result.get("status") == "pass":
        ...
        if ACTIVE_MARKER not in stderr:
            result["failure_reason"] = "deltanet_recurrence_fuse_marker_missing"
            result["status"] = "fail"
    return result
