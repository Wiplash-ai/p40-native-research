#!/usr/bin/env python3
"""Forced-command identity for exact Qwen shared/expert timeline profiling."""
...
SPEC = importlib.util.spec_from_loader("p40_t45_base", SourceFileLoader("p40_t45_base", str(SOURCE)))
...
OUTPUT_TOKENS = 64
EXPECTED_ORIGINAL_COMMAND = "p40-t45-qwen-shared-expert-timeline"
RESULTS_ORIGINAL_COMMAND = "p40-t45-qwen-shared-expert-timeline-results"
ENGINE = Path("/home/jordanculver/p40-native-research/colibri-t45-shared-expert-timeline/c/qwen36")
ENGINE_SHA256 = "ee46ac81fd2c32a8115801951384e75762832c85ad1abd0ae61ede69f063f4c5"
RESULT_DIRECTORY = Path("/home/jordanculver/p40-native-research/results/t45-qwen-shared-expert-timeline")
PROFILE_ID = "t45-exact-qwen-shared-expert-timeline-64"
SCHEMA_VERSION = "p40-t45-qwen-shared-expert-timeline-v1"
...
TIMELINE_LINE = re.compile(
    ...
    rb"[0-9.]+ ms \| dense-end-to-expert-done [+-]?[0-9.]+ ms"
)
...
def run(request: dict) -> dict:
    ...
    if not request["dry_run"] and result.get("status") == "pass":
        ...
        if ACTIVE_MARKER not in stderr:
            ...
            result["status"] = "fail"
        elif b"window collection failed" in stderr or b"event elapsed query failed" in stderr:
            result["failure_reason"] = "shared_expert_timeline_event_failure"
            result["status"] = "fail"
        else:
            match = TIMELINE_LINE.search(stderr)
            if not match or int(match.group(1)) != EXPECTED_WINDOWS:
