#!/usr/bin/env python3
"""Forced-command identity for exact-output real-Qwen W4A8 top-32 shadow."""
...
SPEC = importlib.util.spec_from_loader("p40_t30_base", SourceFileLoader("p40_t30_base", str(SOURCE)))
assert SPEC and SPEC.loader
qwen = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = qwen
SPEC.loader.exec_module(qwen)

EXPECTED_ORIGINAL_COMMAND = "p40-t30-qwen-groupwise-outlier32-shadow"
RESULTS_ORIGINAL_COMMAND = "p40-t30-qwen-groupwise-outlier32-shadow-results"
ENGINE = Path("/home/jordanculver/p40-native-research/colibri-t30-groupwise-outlier32-shadow/c/qwen36")
ENGINE_SHA256 = "f109e0df6d05e00b37b781a27d64fb55a61708dce988fad6553acca270f05c0c"
RESULT_DIRECTORY = Path("/home/jordanculver/p40-native-research/results/t30-qwen-groupwise-outlier32-shadow")
PROFILE_ID = "t30-w4a8-real-expert-groupwise-outlier32-shadow-16"
SCHEMA_VERSION = "p40-t30-qwen-groupwise-outlier32-shadow-v1"
EXPECTED_STDOUT_SHA256 = "43095be2395844a0c7f321ea2496689325d5ae890f91ab53bb31c55e37a0877a"
SHADOW = re.compile(rb"^\[w4a8-groupwise-outlier32-shadow\] rows=(\d+) width=(\d+) rel_l2=([^ ]+) max_abs=([^ ]+) cosine=([^ ]+) finite=(\d+)$", re.MULTILINE)
...
def model_argv() -> list[str]:
    ...
    for item in _base_model_argv():
        ...
        if item == "COLI_CUDA_PROFILE=1":
            output.append("COLI_CUDA_W4A8_DP4A=groupwise-outlier32-shadow")
    return output
...
def run(request: dict) -> dict:
    ...
    if result.get("stdout", {}).get("sha256") != EXPECTED_STDOUT_SHA256:
        ...
        result["failure_reason"] = "groupwise_outlier32_shadow_not_active"
        result["status"] = "fail"
    elif any(match.group(6) != b"1" for match in matches):
        result["failure_reason"] = "groupwise_outlier32_shadow_nonfinite"
        result["status"] = "fail"
    else:
