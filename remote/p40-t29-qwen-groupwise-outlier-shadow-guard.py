#!/usr/bin/env python3
"""Forced-command identity for exact-output real-Qwen W4A8 outlier shadow."""
...
SPEC = importlib.util.spec_from_loader("p40_t29_base", SourceFileLoader("p40_t29_base", str(SOURCE)))
assert SPEC and SPEC.loader
qwen = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = qwen
SPEC.loader.exec_module(qwen)

EXPECTED_ORIGINAL_COMMAND = "p40-t29-qwen-groupwise-outlier-shadow"
RESULTS_ORIGINAL_COMMAND = "p40-t29-qwen-groupwise-outlier-shadow-results"
ENGINE = Path("/home/jordanculver/p40-native-research/colibri-t29-groupwise-outlier-shadow/c/qwen36")
ENGINE_SHA256 = "58e4b8f4bfdebf5cb39fcfb7a685d4e08ba2d0a5dc2d0306ffca617bb42336d4"
RESULT_DIRECTORY = Path("/home/jordanculver/p40-native-research/results/t29-qwen-groupwise-outlier-shadow")
PROFILE_ID = "t29-w4a8-real-expert-groupwise-outlier-shadow-16"
SCHEMA_VERSION = "p40-t29-qwen-groupwise-outlier-shadow-v1"
EXPECTED_STDOUT_SHA256 = "43095be2395844a0c7f321ea2496689325d5ae890f91ab53bb31c55e37a0877a"
SHADOW = re.compile(rb"^\[w4a8-groupwise-outlier-shadow\] rows=(\d+) width=(\d+) rel_l2=([^ ]+) max_abs=([^ ]+) cosine=([^ ]+) finite=(\d+)$", re.MULTILINE)
...
def model_argv() -> list[str]:
    ...
    for item in _base_model_argv():
        ...
        if item == "COLI_CUDA_PROFILE=1":
            output.append("COLI_CUDA_W4A8_DP4A=groupwise-outlier-shadow")
    return output
...
def run(request: dict) -> dict:
    ...
    if result.get("stdout", {}).get("sha256") != EXPECTED_STDOUT_SHA256:
        ...
        result["failure_reason"] = "groupwise_outlier_shadow_not_active"
        result["status"] = "fail"
    elif any(match.group(6) != b"1" for match in matches):
        result["failure_reason"] = "groupwise_outlier_shadow_nonfinite"
        result["status"] = "fail"
    else:
