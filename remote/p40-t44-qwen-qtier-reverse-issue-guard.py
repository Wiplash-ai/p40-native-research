#!/usr/bin/env python3
"""Forced-command identity for exact Qwen reversed expert-device issue order."""
...
SPEC = importlib.util.spec_from_loader("p40_t44_base", SourceFileLoader("p40_t44_base", str(SOURCE)))
...
OUTPUT_TOKENS = 64
EXPECTED_ORIGINAL_COMMAND = "p40-t44-qwen-qtier-reverse-issue"
RESULTS_ORIGINAL_COMMAND = "p40-t44-qwen-qtier-reverse-issue-results"
ENGINE = Path("/home/jordanculver/p40-native-research/colibri-t44-qtier-reverse-issue/c/qwen36")
ENGINE_SHA256 = "69d4b1f51ce998d1487daa8c2b68b65c917953616aa7b4e7f73044234dbd67ea"
RESULT_DIRECTORY = Path("/home/jordanculver/p40-native-research/results/t44-qwen-qtier-reverse-issue")
PROFILE_ID = "t44-exact-qwen-qtier-reverse-issue-64"
SCHEMA_VERSION = "p40-t44-qwen-qtier-reverse-issue-v1"
EXPECTED_STDOUT_SHA256 = "5909fad89de2faac1b77af72bf8b25f56b8f33853df59d35cb04120e9ce1f35f"
ACTIVE_MARKER = b"[qtier-issue-order] reverse device launch order active"
...
def model_argv() -> list[str]:
    ...
    for argument in _base_model_argv():
        ...
        if argument == "COLI_CUDA_PROFILE=1":
            result.append("COLI_QTIER_REVERSE_ISSUE=1")
    return result
...
def run(request: dict) -> dict:
    ...
    if not request["dry_run"] and result.get("status") == "pass":
        ...
        if ACTIVE_MARKER not in stderr:
            result["failure_reason"] = "qtier_reverse_issue_marker_missing"
            result["status"] = "fail"
    return result
