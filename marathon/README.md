...
This controller is the safe bridge between Wiphand scheduling and a native Codex thread. It does **not** use Wiphand's `codex_article` executor: that executor creates a fresh unsandboxed article job and cannot resume a native thread. Wiphand only writes a fixed queue record; the user-level host service performs the quota check and native turn.

The controller uses the local Codex app-server protocol to read the ChatGPT-backed Codex rate limit, refuse low headroom, start one native thread with a thread goal, and dispatch exactly one gated task per run. Stdio app-server threads are process-local before their first turn, so every scheduled task gets its own native goal thread. It never redeems rate-limit reset credits. State and JSONL protocol logs live under this directory and are intentionally ignored by Git.
...
The initial Wiphand executor mounts only the research repository. It receives neither Codex credentials, SSH material, nor `/dev/nvidia*` devices. A later, separately reviewed T00 executor will receive a dedicated forced-command read-only telemetry key, never the user's general SSH directory. T02+ must run guarded GPU commands on the server itself after the documented gates pass.

## Local commands

```sh
scripts/codex_marathon.py status
scripts/codex_marathon.py prepare
